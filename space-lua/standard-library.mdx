---
title: "Standard Library"
description: "Built-in functions and modules available in Space Lua, including Lua 5.4 standard library plus SilverBullet extensions."
---

Space Lua includes a comprehensive standard library based on Lua 5.4, with additional extensions useful for text processing and note-taking workflows.

## Global functions

These are available without any module prefix:

| Function | Description |
|---|---|
| `print(...)` | Print to the log (browser console or server log) |
| `type(v)` | Returns the type of a value as a string |
| `tostring(v)` | Converts a value to its string representation |
| `tonumber(s)` | Converts a string to a number |
| `assert(expr, msg?)` | Raises an error if `expr` is falsy |
| `error(msg)` | Throws an error |
| `pcall(fn, ...)` | Calls a function in protected mode, catching errors |
| `xpcall(fn, handler)` | Like `pcall` but with a custom error handler |
| `pairs(t)` | Iterator over all key-value pairs in a table |
| `ipairs(t)` | Iterator over integer-keyed entries in order |
| `unpack(t)` | Unpacks a table into individual values |
| `setmetatable(t, mt)` | Sets the metatable for a table |
| `getmetatable(t)` | Gets the metatable of a table |
| `rawset(t, k, v)` | Sets a table key bypassing metamethods |
| `dofile(path)` | Loads and executes a `.lua` file from your space |

**SilverBullet-specific globals:**

| Function | Description |
|---|---|
| `each(t)` | Iterator over values only (no indices) |
| `some(v)` | Returns `nil` if value is "empty" (empty table, whitespace-only string, `inf`, `nan`), otherwise returns the value unchanged |

`some()` is particularly useful in templates:

```lua
print(some("hello"))        -- hello
print(some(""))             -- nil
print(some({}) or "empty")  -- empty
```

## `string`

Standard Lua string operations. Since strings have `string` as their metatable, you can call these as methods: `s:startsWith("h")`.

**SilverBullet extensions:**

| Function | Description |
|---|---|
| `string.split(s, sep)` | Splits a string by a separator |
| `string.startsWith(s, prefix)` | Tests if a string starts with a prefix |
| `string.endsWith(s, suffix)` | Tests if a string ends with a suffix |
| `string.trim(s)` | Strips whitespace from both ends |
| `string.trimStart(s)` | Strips leading whitespace |
| `string.trimEnd(s)` | Strips trailing whitespace |
| `string.matchRegex(s, pattern)` | Matches against a JavaScript regex |
| `string.matchRegexAll(s, pattern)` | Iterator over all JavaScript regex matches |

<Warning>
  Standard Lua `string.find`, `string.match`, `string.gmatch`, and `string.gsub` use **Lua patterns**, not regular expressions. Use `matchRegex`/`matchRegexAll` when you need full regex support.
</Warning>

## `table`

Table manipulation with SilverBullet extensions:

| Function | Description |
|---|---|
| `table.insert(t, v)` | Appends a value to an array table |
| `table.remove(t, i?)` | Removes and returns value at index |
| `table.sort(t, fn?)` | Sorts a table in-place |
| `table.concat(t, sep?)` | Joins array table into a string |
| `table.keys(t)` | Returns an array of all keys |
| `table.includes(t, value)` | Checks if a list contains a value |
| `table.find(t, fn, from?)` | Finds first element matching a predicate |
| `table.select(t, keys...)` | Returns a new table with only selected keys |

```lua
-- Select specific fields from an object
local summary = table.select(page, "name", "lastModified")

-- Check membership
if table.includes(page.tags, "project") then
  -- ...
end
```

## `math`

Standard Lua math functions (`abs`, `ceil`, `floor`, `max`, `min`, `sqrt`, `sin`, `cos`, etc.) plus:

| Function | Description |
|---|---|
| `math.cosineSimilarity(a, b)` | Cosine similarity between two numeric vectors |

## `os`

Date and time functions:

| Function | Description |
|---|---|
| `os.time(table?)` | Current Unix timestamp, or timestamp for a specific date |
| `os.date(format?, timestamp?)` | Formats a timestamp as a string |

```lua
-- Current date as YYYY-MM-DD
local today = os.date("%Y-%m-%d")

-- Timestamp for a specific date
local ts = os.time({year=2024, month=1, day=15})
```

## `encoding`

| Function | Description |
|---|---|
| `encoding.base64Encode(data)` | Encode data as base64 |
| `encoding.base64Decode(s)` | Decode a base64 string |
| `encoding.utf8Encode(s)` | Encode a UTF-8 string to bytes |
| `encoding.utf8Decode(data)` | Decode bytes to a UTF-8 string |

## SilverBullet globals

In addition to Lua standard library, Space Lua has access to these SilverBullet-provided globals:

| Global | Description |
|---|---|
| `index` | Query the object index — see [Index API](/api/index) |
| `query` | LIQ query function — see [Lua Integrated Query](/space-lua/lua-integrated-query) |
| `template` | Template creation and rendering — see [Template API](/api/template) |
| `templates` | Pre-built template functions (`pageItem`, `taskItem`, etc.) |
| `space` | Read/write pages and files — see [Space API](/api/space) |
| `editor` | Interact with the editor UI — see [Editor API](/api/editor) |
| `widget` | Create rendered widgets — see [Widgets](/space-lua/widgets) |
| `widgets` | Pre-built widgets (buttons, subPages, etc.) |
| `event` | Subscribe to and emit events |
| `config` | Read and write configuration values |
| `js` | JavaScript interop — see [JavaScript Interop](/space-lua/javascript-interop) |
